...
def unfollow(username):
    ...
    db.session.commit()
    flash(f'Вы отказались от подписки на {username}!')
    return redirect(url_for('user', username=username))
...
def index():
    ...
    return render_template("index.html", title='Главная', form=form, posts=posts.items, 
...
def explore():
    ...
    return render_template('index.html', title='Поток', posts=posts.items,
        next_url=next_url, prev_url=prev_url)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_password_request.html', title='Сбросить пароль', form=form)
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash('Пароль был успешно изменён.')
        return redirect(url_for('login'))
